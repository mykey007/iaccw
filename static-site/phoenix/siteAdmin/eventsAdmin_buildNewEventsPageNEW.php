	...
	if ($num_rows == 0) header("Location:eventsAdmin_listings.php");
		...
		while ($a_row = mysql_fetch_row($result)) { // 							while there is a row of events data
			...
			foreach ($a_row as $key => $field) { //								for each row of event data
				$date = explode("-",$a_row[1]); //								convert the date to an array of 3 separate variables
				...
				fwrite($Events, $a_row[2]); //									write the event title
				...
				fwrite($Events, $a_row[4]); //									write the event summary
		...
		$theFilesize = filesize($eventsEventTitle);
